...
# TCP/UDP packet data
Packet = namedtuple('Packet', ['type', 'source_ip', 'source_port', 'dest_ip', 'dest_port', 'length'])
...
def _clean_old_sessions(stats, session_timeout):
...
def process_sessions(device, stats, running, session_timeout):
    ...
    for packet in pcap_object:
        _clean_old_sessions(stats, session_timeout)
        if packet is None:
            continue
...
class NetworkSessions(Process):
    def __init__(self, device, session_timeout=10):
    ...
    def run(self):
        self._running.value = True
        process_sessions(self._device, self._stats, self._running, self._session_timeout)
